...
export const PlayerModalFrame: React.FC<PlayerModalFrameProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[10050] flex min-h-0 items-end justify-center bg-[#102f38]/78 px-0 py-0 backdrop-blur-sm sm:items-center sm:px-4 sm:py-6">
      <section
        role="dialog"
        aria-modal="true"
        aria-labelledby="player-modal-title"
        className="relative flex h-[100dvh] max-h-[100dvh] w-full min-h-0 flex-col overflow-hidden border border-[#d8c29a] bg-[#fffaf2] text-[#293a38] shadow-[0_28px_80px_-32px_rgba(16,47,56,0.85)] motion-reduce:transition-none sm:h-auto sm:max-h-[min(92dvh,860px)] sm:max-w-[720px] sm:rounded-[28px]"
      >
        <div className={`h-1.5 shrink-0 bg-gradient-to-r ${accentStyle.bar}`} />

        <header className="flex shrink-0 items-start gap-3 border-b border-[#ead7b8] px-5 pb-4 pt-[calc(var(--safe-top)+1.1rem)] sm:px-7 sm:pt-5">
        ...
        {footer && (
          <footer className="shrink-0 border-t border-[#ead7b8] bg-[#fffaf2]/96 px-5 pb-[calc(var(--safe-bottom)+1rem)] pt-4 backdrop-blur-md sm:px-7 sm:pb-5">
            {footer}
          </footer>
        )}
      </section>
