...
export const GameStats: React.FC<GameStatsProps> = ({ gameState, summary, onOpenHappiness }) => {
    return (
        <div className="w-full">
            {/* Happiness Card */}
            <Card className="p-3 relative overflow-hidden bg-slate-900 border-slate-700 cursor-pointer hover:border-pink-500/50 transition-all group" onClick={onOpenHappiness}>
                <div className="relative z-10 flex flex-col justify-between h-full">
                    <div>
                        <h3 className="text-slate-400 text-xs font-bold uppercase tracking-[0.2em] mb-1 flex items-center gap-2">
                            <Heart size={14} className="text-pink-500" />
                            幸福指數
                        </h3>
                        <div className="flex items-baseline gap-2 mb-1.5">
                            <div className="text-4xl font-black text-white tracking-tighter"> {gameState.happinessTotal} </div>
                    ...
                    <div className="text-[10px] text-slate-500 font-mono mt-1.5 uppercase tracking-[0.3em]"> 邁向幸福人生 </div>
                </div>
                <div className="absolute right-4 top-1 flex flex-col items-center group-hover:scale-110 transition-transform duration-500">
                    <div className="relative w-20 h-20 flex items-center justify-center">
                        ...
                        <div className="absolute inset-0 flex items-center justify-center">
                            <Heart size={56} className="text-slate-700/30" strokeWidth={1.5} />
                        </div>
                        <div
                        ...
                        >
                            <div className="absolute bottom-0 left-0 w-20 h-20 flex items-center justify-center">
                                <Heart
                                    size={56}
                        ...
                        <div className="absolute top-6 left-7 w-2.5 h-1.5 bg-white/30 rounded-full blur-[1px] z-20 rotate-[-20deg]" />
                    </div>
                </div>
            </Card>
        </div>
