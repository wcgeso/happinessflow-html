    ...
    <div className="fixed inset-0 bg-slate-950 flex flex-col overflow-hidden animate-in fade-in duration-500 z-50">
      ...
      <div className="flex-1 relative flex flex-col justify-center overflow-hidden bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-slate-900 to-slate-950">
        ...
        <div 
            ...
            className="w-full flex items-center overflow-x-auto snap-x snap-mandatory gap-6 pb-4 pt-16 hide-scrollbar h-full justify-start md:justify-center px-[30%]"
...
export const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, (firebaseUser) => {
      if (firebaseUser) {
        setUser({ email: firebaseUser.email || '', name: firebaseUser.displayName || firebaseUser.email?.split('@')[0] || 'Player' });
        if (currentView === 'auth_home' || currentView === 'login' || currentView === 'register') { setCurrentView('lobby'); }
      } else {
        setUser(null);
        if (currentView !== 'auth_home' && currentView !== 'login' && currentView !== 'register') { setCurrentView('auth_home'); }
      }
  ...
  const handleLogin = async () => {
      ...
      if(authEmail.trim() && (authPassword.trim() || !auth)) {
          if (auth) {
              try { await signInWithEmailAndPassword(auth, authEmail, authPassword); setAuthEmail(''); setAuthPassword(''); } catch (error: any) { setAuthError(error.message || '登入失敗'); }
          } else { setUser({ email: authEmail, name: authEmail.split('@')[0] }); setCurrentView('lobby'); }
      } else { setAuthError('請輸入帳號和密碼'); }
  };
