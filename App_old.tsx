...
import { Button, Card, Input } from './src/components/ui/ui';
import { FinancialStatement } from './src/components/business/FinancialStatement';
import { TransactionForm, TransactionData } from './src/components/business/TransactionForm';
import { HappinessPanel } from './src/components/business/HappinessPanel';
...
export const App: React.FC = () => {
    ...
    const getCurrentAvatar = () => {
        ...
        return avatar ? avatar.icon : <User size={24} className="text-yellow-400" />;
    };
            ...
            <div className="min-h-screen bg-slate-950 p-6 flex flex-col">
                <div className="flex justify-between items-center mb-8 border-b border-slate-800 pb-4">
                    <div className="flex items-center gap-3">
                        <div
                            className="w-12 h-12 rounded-full bg-slate-800 border-2 border-yellow-500 flex items-center justify-center cursor-pointer hover:border-yellow-400 transition-colors"
    ...
    return (
        <div className="min-h-screen bg-slate-950 pb-20 overflow-x-hidden animate-in fade-in duration-500">
            ...
            {alertInfo && (
                <div className={`fixed bottom-28 left-1/2 -translate-x-1/2 z-[100] px-6 py-3 rounded-full shadow-2xl whitespace-nowrap flex items-center gap-2 animate-in slide-in-from-bottom-4 border ${alertInfo.type === 'error' ? 'bg-rose-900 border-rose-500 text-rose-100' :
                ...
                <div className="max-w-7xl mx-auto flex items-center justify-between relative">
                    <div className="flex items-center gap-3">
                        {gameState.profession && (
                            <>
                                <div
                                    className="w-10 h-10 rounded-full bg-slate-800 border-2 border-yellow-500 flex items-center justify-center cursor-pointer hover:border-yellow-400 transition-colors"
                                    onClick={() => setShowRankListModal(true)}
                                >
                                    {/* 職業圖示 mapping */}
                                    {gameState.profession.id === 'pilot' && <Plane size={20} className="text-yellow-400" />}
                                    {gameState.profession.id === 'doctor' && <Stethoscope size={20} className="text-yellow-400" />}
                                    {gameState.profession.id === 'artist' && <Palette size={20} className="text-yellow-400" />}
                                    {gameState.profession.id === 'honey_brewer' && <Coffee size={20} className="text-yellow-400" />}
                                    {gameState.profession.id === 'teacher' && <BookOpen size={20} className="text-yellow-400" />}
                                    {gameState.profession.id === 'technician' && <Wrench size={20} className="text-yellow-400" />}
                                    {gameState.profession.id === 'accountant' && <Calculator size={20} className="text-yellow-400" />}
                                    {gameState.profession.id === 'clerk' && <ShoppingBag size={20} className="text-yellow-400" />}
                                    {gameState.profession.id === 'nest_builder' && <Hammer size={20} className="text-yellow-400" />}
                                    {gameState.profession.id === 'fashion_designer' && <Scissors size={20} className="text-yellow-400" />}
                                </div>
                                    ...
                                    <div className="flex items-center gap-0.5">
                                        {Array.from({ length: 5 }).map((_, i) => (
                                            <Star
                                                key={i}
                                                size={12}
                                                className={i < gameState.currentRankLevel ? 'text-yellow-400 fill-yellow-400' : 'text-slate-600'}
                                            />
                                        ))}
                                    </div>
                                    <span className="text-xs text-slate-400">{gameState.currentRankTitle}</span>
                                </div>
                            </>
                        )}
                    </div>
