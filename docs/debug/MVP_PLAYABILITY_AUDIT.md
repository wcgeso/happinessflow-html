...
> 現有《第二人生》Legacy 專案，距離「一位執行師 + 至少兩位玩家，可連續完成至少 3 個完整 Round」還差哪些會阻塞 MVP 可玩的問題。
...
| P1-04（部分修正 2026-07-07） | 規則所有權分裂在 resolver 與畫面流程 | 同一張卡的正式規則同時存在 current-player resolver 與 shared prompt 特判，容易重複處理或漏處理 | 逐一核實後發現：`asset_sale` 類共享卡其實已經正確共用 `resolveBoardCardAction()`（`sharedCardLocalAction` 就是直接呼叫它）；但 `cash_dividend`／`stock_dividend` 這兩種共享卡，`GameView.tsx` 原本另外用 `asset.name.match(/[A-Z]\d+/)` 手刻一份股利/配股計算，跟 `boardCardActions.ts` 內部用的 `stockSymbolFromAssetName()`（有股票代碼備援邏輯）不同，對於名稱沒有標準代碼格式的股票資產，兩邊會算出不同結果（畫面顯示 0，但正式規則其實有股利）——這是真實的規則分裂與潛在漏發股利。已修正：`sharedDividendAmount`／`sharedStockDividendItems` 改為直接讀取 `sharedCardLocalAction`（`resolveBoardCardAction()` 的結果）算出的 `txData.amount`／`txData.stockDividendPayload.items`，不再另外重算。**未涵蓋範圍**：市場行情（`kind === 'market'`）卡片的房市/企業相關 shared 分流，以及未來新增的其他共享卡型，仍需個別確認是否也有共用同一 resolver，此次未逐一排查完 | `src/utils/boardCardActions.ts`, `src/views/game/GameView.tsx` |
| P1-05（範圍內修正 2026-07-07） | 金融 / 幸福效果缺乏事件級 idempotency | 若 modal 重開、effect watcher 重跑、snapshot 延遲，可能重複扣款或重複加幸福 | ~~沒有以 eventId / cardKey 為正式結算 token~~ 已修正棋盤財務檢核流程（`BoardFinancialCheckModal` → `applyResolvedBoardFinancialTx()`）：新增 `GameState.appliedBoardFinancialEventIds`，`handleTransactionSubmit()` 以 `data.boardEventId`（= `boardState.currentEvent.id`）查表，已處理過的事件直接視為成功、不重複套用，且此陣列會持久化（localStorage + 同步至 room），可防禦跨畫面重掛載的重複套用，非僅元件內 state。**未涵蓋範圍**：本次只修財務檢核這條主流程，其他非經此流程的金融/幸福套用點（例如 `executeMedicalClaim`、`executeAircraftClaim`、`executePromotionFee` 等各自獨立的 `newState.history` 寫入點）仍各自依賴自己原有的一次性 guard，未套用同一機制，仍是分散防護、非全系統統一 token | `src/views/game/GameView.tsx`, `src/hooks/useGameLogic.ts`, `src/types.ts` |
...
| RM-04（已修正 2026-07-07） | 卡片效果必須 `Draw → Reveal → Resolve → Complete` | ~~市場新聞在 `activeBoardCardAction.kind === market` 時即同步 `room.marketPrices`，未明確等待 Reveal~~ 已修正：確認此為真實缺陷（非過時記錄）——套用行情的 `useEffect`（`GameView.tsx`）原本只要抽到卡就立刻寫入 `room.marketPrices`（全房間共享可見），導致其他玩家在抽卡玩家翻牌前就能看到新股價。已加上 `isBoardCardRevealed` 檢查，翻牌前不再寫入房間行情 | `GAME_OVERVIEW` / `GAME_SYSTEM_MAP` vs `GameView.tsx`（已對齊） |
...
| RM-06（已修正 2026-07-07，並依 CARD_DECISION.md DEC-005 定案） | 創業貸款 Follow-up 應是正式事件鏈的一部分 | 稽核當下重新核實：`pendingStartupUpgradeAction` 實際上已存於 `room.playerStates[uid]`（非純本地 state），並掛在正式的 Bank Event（Payday 流程）觸發點上，這部分與文件描述有落差。~~Legacy 以玩家本地 `pendingStartupUpgradeAction` 表示，未進入正式 shared event / queue~~ 依 DEC-005，產品已裁定保留現行「每次經過銀行都可重新擲骰嘗試」設計，不需另建獨立事件類型。但發現一個真實缺陷並已修正：`BizUpgradeModal` 擲骰未達 5（或玩家未擲骰即關閉）時，`onClose` 仍會呼叫 `clearPendingStartupUpgradeAction()`，導致待升級狀態被永久清除，玩家下次經過銀行無法再嘗試，直接牴觸 DEC-005「擲骰未達標時應保留待升級狀態」。已修正：`onClose` 不再清除待升級狀態，只清除於 `onUpgrade`（升級成功）路徑 | `GAME_OVERVIEW` / `GAME_SYSTEM_MAP` vs `GameView.tsx`（已對齊 DEC-005） |
| RM-07（已修正 2026-07-07） | 醫院事件應為「第二次擲骰 → 支付醫療費 → 停回合 1 次」 | ~~landing hospital 時已先寫入 `skipTurns`，早於醫療費正式結算~~ 已修正：確認此為真實缺陷。`buildBoardMovementResolution()` 不再於棋子抵達當下寫入 `skipTurns`，改為在事件上記錄 `hospitalSkipTurns`；`advanceBoardEventQueue('hospital')`（醫療費財務檢核完成後才會呼叫）才實際把停回合數寫入 `boardState.skipTurns`，與 `docs/gdd/HOSPITAL_SYSTEM.md` 完成規則（醫療費正式成立後才寫入停回合）對齊 | `GAME_OVERVIEW` vs `buildBoardMovementResolution()`（已對齊）；`src/types.ts`、`src/context/RoomContext.tsx` |
