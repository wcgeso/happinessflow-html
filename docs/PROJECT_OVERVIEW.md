...
HappinessFlow 是一個以 Firebase 為後端即服務的前端單體應用。核心設計是：玩家本機負責計算自己的財務狀態，再把完整 `GameState` 寫入單一房間文件；所有玩家、執行師及大地圖透過同一份 Firestore snapshot 同步。這使部署簡單且即時畫面一致，但單文件集中、非交易式併發更新及客戶端權威是後續擴充多人規模時最需要優先處理的架構邊界。
